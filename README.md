...
```sh
$ docker network create -d overlay rbee-monitoring
```
...
```sh
$ docker service create --name rbee-monitoring-master --network rbee-monitoring -p 9042:9042 -p 9200:9200 -p 9300:9300 rbee-monitoring:<tag>
```
...
```sh
$ docker service create --name rbee-monitoring-master --network rbee-monitoring rbee-monitoring:<tag>
$ docker service create --name rbee-monitoring-slave --network rbee-monitoring -p 9042:9042 -p 9200:9200 -p 9300:9300 rbee-monitoring:<tag>
```
After a few moments, the rbee-monitoring-slave instance should find the other for both Cassandra and Elasticsearch.  
Furthermore, the ports 9042, 9200 and 9300 on any Docker-swarm host will forward/load-balance to any of the running rbee-monitoring-slave instances.
...
To scale the service, only the _rbee-monitoring-slave_ may be upscaled by one instance at a time.  
Cassandra doesn't like multiple joining instances at once.
```sh
$ docker service scale rbee-monitoring-slave=2
```
 ...
 * **RBEE_MASTER_HOST** _(default: tasks.rbee-monitoring-master)_  
